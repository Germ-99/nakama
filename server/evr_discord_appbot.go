...
// DiscordAppCommand is a struct that holds the discordgo application command and the handler function for it.
	...
	return []*DiscordAppCommand{
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userID string, groupID string) error {
				...
				if err := func() error {
					...
					if _, err := d.nk.MatchSignal(ctx, matchID.String(), data); err != nil {
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userID string, groupID string) error {
				...
				if err := func() error {
					...
					// Clear the password
					return d.nk.UnlinkEmail(ctx, userID, account.GetEmail())
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userID string, groupID string) error {
				...
				if _, err := MatchLabelByID(ctx, d.nk, matchID); err != nil {
					return fmt.Errorf("failed to get match: %w", err)
				}

				// Set the next match ID
				if err := SetNextMatchID(ctx, d.nk, userID, matchID, AnyTeam, ""); err != nil {
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userIDStr string, groupID string) error {
				metadata, err := EVRProfileLoad(ctx, d.nk, userIDStr)
				if err != nil {
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userIDStr string, groupID string) error {
				...
				// Set the metadata
				md, err := EVRProfileLoad(ctx, d.nk, userIDStr)
				if err != nil {
					return err
				}
				md.SetActiveGroupID(uuid.FromStringOrNil(groupID))

				if err := EVRProfileUpdate(ctx, d.nk, userIDStr, md); err != nil {
					return err
				}
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userIDStr string, groupID string) error {
				...
				if err := StorageRead(ctx, d.nk, userIDStr, loginHistory, false); err != nil {
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userIDStr string, groupID string) error {
				...
				isGlobalOperator, err := CheckSystemGroupMembership(ctx, d.db, userIDStr, GroupGlobalOperators)
				if err != nil {
					return fmt.Errorf("error checking global operator status: %w", err)
				}
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userID string, groupID string) error {
				...
				if md, err := GuildGroupLoad(ctx, d.nk, groupID); err != nil {
					return fmt.Errorf("failed to load guild group metadata: %w", err)
				} else {
					md.CommandChannelID = i.ChannelID
					if err := GuildGroupStore(ctx, d.nk, d.guildGroupRegistry, md); err != nil {
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userID string, groupID string) error {
				...
				go func() {
					...
					for {
						...
						members, err = d.nk.StreamUserList(StreamModeParty, partyStr, "", d.pipeline.node, false, true)
						if err != nil {
						...
						currentlyMatchmaking, err := d.nk.StreamUserList(StreamModeMatchmaking, groupID, "", "", false, true)
						if err != nil {
							logger.Error("Failed to list stream users", zap.Error(err))
							return
						}
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, callerMember *discordgo.Member, userID string, groupID string) error {
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userID string, groupID string) error {
				...
				case "assign":
					...
					account, err := d.nk.AccountGetId(ctx, targetUserID)
					if err != nil {
					...
					if err := AssignEntitlements(ctx, logger, d.nk, userID, i.Member.User.Username, targetUserID, "", entitlements); err != nil {
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userID string, groupID string) error {
				...
				presences, err := d.nk.StreamUserList(uint8(mode), subject, subcontext, label, includeHidden, includeOffline)
				if err != nil {
		...
		},
		{
			...
			handler: func(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userID string, groupID string) error {
				...
				case "members":
					...
					partyMembers, err := d.nk.StreamUserList(StreamModeParty, partyUUID.String(), "", d.pipeline.node, false, true)
					if err != nil {
